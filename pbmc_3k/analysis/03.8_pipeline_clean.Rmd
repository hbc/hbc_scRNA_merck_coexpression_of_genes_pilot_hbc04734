...
Getting a the list of surfaceome genes in the dataset and filtering out any genes that are found in fewer than `r min_cells`
...
surfaceome_genes_all <- names(num_cells[num_cells > min_cells])
...
Here I am showing all the surfaceome genes we have to work with in the dataset. Showing their average expression and the number of cells they are expressed.

Looking at top 50 and bottom 50 genes.
...
filter_genes <- filter_genes$gene
...
for (method_1 in methods) {
    for (method_2 in methods) {
        ...
        df <- df %>% subset((Var1 %in% markers[["B"]]) & (Var2 %in% markers[["B"]]))
